...
public class Parser {
	...
	public OutlookData[] getOutlookDataArray() throws IOException{
		...
		while(fileReader.hasNextLine()){
			...
			outlookArr[count++] = new OutlookData(Integer.parseInt(splitArr[0].substring(4)),splitArr[1],Integer.parseInt(splitArr[2]),splitArr[3],splitArr[4],splitArr[5],splitArr[6],Integer.parseInt(splitArr[7]),splitArr[8],Integer.parseInt(splitArr[9]), splitArr[10], 0);
		}
		//Close the stream
		fileReader.close();
		return outlookArr;
	...
	public CityData[] getCityDataArray() throws IOException{
		...
		while(fileReader.hasNextLine()){
			...
			cityArr[count++] = new CityData(Integer.parseInt(splitArr[0]),splitArr[1],splitArr[2], splitArr[3], splitArr[4], Double.parseDouble(splitArr[5]), Double.parseDouble(splitArr[6]));
		}
		//Close the stream
		fileReader.close();
		return cityArr;
